...
`anchor-sys-kernel-core` is centered on systems programming work in R. The technical target is: Build an R toolkit that studies kernel behavior through layout fixtures, with stable geometry snapshots and no credentials or hosted services.
...
The score is deterministic. The current threshold is 184. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 2.
