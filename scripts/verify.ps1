...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-anchor-sys-kernel-detail.ps1

Rscript tests/test_domain_review.R

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-anchor-sys-kernel-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-anchor-sys-kernel-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-anchor_sys_kernel_core-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
